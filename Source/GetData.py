...
with open(OutWeaponsPath, 'w') as f:
    for gun in guns:
        if gun['range'] == 'Melee':
            ...
            f.write('\t\tself.S = {}\n'.format(gun['s']) if '+' not in gun['s'] and 'x' not in gun['s'] and '-' not in gun['s'] and gun['s'] != '4' else '\t\tself.S {}= {}\n'.format(gun['s'][0], gun['s'][1:]))
...
for unit in units:
    ...
    index += 1 if '"' not in unit[2] else 2  # Si hay más de una unidad en el escuadron definida
    ...
    for point in pointsfile:
        d = 0
        if re.search(r"\d", point).start() is not None:
            d = re.search(r"\d", point).start()
...
with open(OutUnitsPath, 'w') as f:
    for auxUnit in auxUnits:
        f.write('class {}({}, Human):\n'.format(auxUnit['name'].replace(' ', ''), 'Infantry' if 'INFANTRY' in auxUnit['keywords'] else 'Vehicle'))
        f.write('\tdef __init__(self):\n')
        f.write('\t\tsuper().__init__()\n')
        ...
        f.write('\t\tself.Gun1 = Weapons.{}()\n'.format(auxUnit['gun1'].replace(' ', ''))) if 'gun1' in auxUnit.keys() else None
        f.write('\t\tself.POINTS += self.Gun1.POINTS\n'.format(auxUnit['gun1'].replace(' ', ''))) if 'gun1' in auxUnit.keys() else None
        f.write('\t\tself.Gun2 = Weapons.{}()\n'.format(auxUnit['gun2'].replace(' ', ''))) if 'gun2' in auxUnit.keys() else None
        f.write('\t\tself.POINTS += self.Gun2.POINTS\n'.format(auxUnit['gun2'].replace(' ', ''))) if 'gun2' in auxUnit.keys() else None
        f.write('\t\tself.Gun3 = Weapons.{}()\n'.format(auxUnit['gun3'].replace(' ', ''))) if 'gun3' in auxUnit.keys() else None
        f.write('\t\tself.POINTS += self.Gun3.POINTS\n'.format(auxUnit['gun3'].replace(' ', ''))) if 'gun3' in auxUnit.keys() else None
        f.write('\t\tself.Gun4 = Weapons.{}()\n'.format(auxUnit['gun4'].replace(' ', ''))) if 'gun4' in auxUnit.keys() else None
        f.write('\t\tself.POINTS += self.Gun4.POINTS\n'.format(auxUnit['gun4'].replace(' ', ''))) if 'gun4' in auxUnit.keys() else None
        f.write('\t\tself.Gun5 = Weapons.{}()\n'.format(auxUnit['gun5'].replace(' ', ''))) if 'gun5' in auxUnit.keys() else None
    ...
    if index_abilities is not None and index_wargear is not None:
        for i in range(index_wargear, index_abilities):
            ...
            if point != -1:
                ...
                wargear_options[len(wargear_options) - 1] += unit[i]
    faction_keywords = unit[len(unit)-2].replace('FACTION KEYWORDS ', '').split(',')
    keywords = unit[len(unit)-1].replace('KEYWORDS ', '').split(',')
    d = 0
    if re.search(r"\d", unit[1]) is not None:
        d = re.search(r"\d", unit[1]).start()
    squad['name'] = unit[1][:d - 1].replace(' ', '')
    squad['keywords'] = keywords
    squad['factionkeywords'] = faction_keywords
